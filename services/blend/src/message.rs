...
use lb_blend::{
    ...
    network::core::message::SessionBoundEncapsulatedMessageWithVerifiedHeader,
};
...
pub enum ProcessedMessage<BroadcastSettings> {
    ...
    LocallyGenerated(Box<EncapsulatedMessageWithVerifiedPublicHeader>),
    ReceivedAndProcessed(Box<SessionBoundEncapsulatedMessageWithVerifiedHeader>),
}
    ...
    for ProcessedMessage<BroadcastSettings>
{
    fn from(value: EncapsulatedMessageWithVerifiedPublicHeader) -> Self {
        Self::LocallyGenerated(Box::new(value))
    }
}
